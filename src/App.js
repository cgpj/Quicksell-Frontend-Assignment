...
function App() {
  ...
  const [orderValue, setorderValue] = useState(
    getStateFromLocalStorage() || 'title'
  );
  ...
  function getStateFromLocalStorage() {
    const storedState = localStorage.getItem('Value');
    if (storedState) {
        ...
        for (let i = 0; i < response.data.tickets.length; i++) {
          for (let j = 0; j < response.data.users.length; j++) {
            if (response.data.tickets[i].userId === response.data.users[j].id) {
              ...
              if (!extractedUserList.includes(response.data.users[j].name)) {
                extractedUserList.push(response.data.users[j].name);
              }
            }
          }
        }
      }
      await setUserList(extractedUserList);
